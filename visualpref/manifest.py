...
``resolve_item`` 负责反向解析。两者同处本模块，保证"媒体 -> 条目"契约的
唯一实现，也避免与 ``paths``/``items`` 形成循环导入。
"""
...
def resolve_item(
    ...
    """解析媒体对应的 ``MediaItem``（视频=帧目录条目，图片=单文件条目）。
    ...
    for key in (path_key(p), str(p.absolute()), str(p)):
        ...
        if val is not None and manifest_dir_of(val):
            return MediaItem.from_entry_path(root / manifest_dir_of(val), root)
    if is_image(p):
        return MediaItem.from_entry_path(
            root / config.FRAMES_IMAGE_SUBDIR / (sanitize_name(p.stem) + p.suffix.lower()),
            root,
        )
    return MediaItem.from_entry_path(root / config.FRAMES_VIDEO_SUBDIR / sanitize_name(p.stem), root)
...
def item_key_for(
    ...
    """返回媒体条目的相对子路径 key（``video/foo`` / ``image/foo.png``）。

    供缺帧判定等场景与 ``FramesNamer``/扫描产出的 key 集合比对。
    """
    return resolve_item(media_path, frames_root, manifest).key
...
class FramesNamer:
    ...
    - 图片条目 -> ``image/{sanitized_stem}{out_ext}``（单文件）：``image_size>0``
      时输出 ``.jpg``（摄入端 center-crop 重编码为 JPEG），否则保留源扩展名（字节复制）。
    ...
    def assign(self, media_path: Path) -> MediaItem:
        key = path_key(media_path)
        if key in self.manifest:
            return MediaItem.from_entry_path(
                self.frames_root / manifest_dir_of(self.manifest[key]), self.frames_root
            )
        p = Path(media_path)
        stem = sanitize_name(p.stem)
        src_ext = p.suffix.lower()
        if is_image(p):
            # 输出扩展名：center-crop 重编码为 JPEG；否则保留源扩展名（字节复制）。
            # 与摄入端 ingest_image 的输出保持一致（见 frames.py）。
            out_ext = ".jpg" if self.image_size and self.image_size > 0 else src_ext
