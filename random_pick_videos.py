#!/usr/bin/env python
"""从目录下随机选取若干媒体文件（视频/图片），打印到标准输出或写入文件。
...
def find_videos(root: Path, recursive: bool = True) -> list[Path]:
    """递归/非递归查找 root 下所有媒体文件（视频 + 图片），返回绝对路径列表（升序）。"""
    root = root.resolve()
    if recursive:
        files = (p for p in root.rglob("*") if p.is_file())
    else:
        files = (p for p in root.iterdir() if p.is_file())
    return sorted(p for p in files if p.suffix.lower() in (VIDEO_EXTS | IMAGE_EXTS))


def pick(videos: list[Path], count: int, seed: int | None) -> list[Path]:
    """从媒体列表中随机选取至多 count 个；seed 提供时结果可复现。"""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="从目录下随机选取若干媒体文件（视频/图片）")
...
def main(argv=None) -> None:
    ...
    print(f"共找到 {len(videos)} 个媒体文件")

    if not videos:
        raise SystemExit("目录中没有找到任何媒体文件")
    ...
    if args.output:
        ...
        print(f"已随机选取 {len(picked)} 个媒体 -> {out}")
    else:
        print(f"随机选取 {len(picked)} 个媒体：")
        for line in lines:
            print(line)
