...
- 纯路径/枚举工具在本模块；
- 媒体条目抽象（``MediaItem``，含 key/kind/frame_paths）见 ``items.py``；
- manifest 与 ``FramesNamer``/``resolve_item`` 见 ``manifest.py``。
...
def path_key(media_path: Path) -> str:
    """稳定的媒体路径 key（绝对规范化路径），用于 manifest 与 hash。"""
    return str(Path(media_path).resolve())


def short_hash(media_path: Path, length: int = 8) -> str:
    """基于完整路径的稳定短哈希，用于同名媒体去重后缀。"""
    return hashlib.sha256(path_key(media_path).encode("utf-8")).hexdigest()[:length]
...
def feature_cache_path(cache_dir: Path, item_key: str, suffix: str = ".pt") -> Path:
    """返回某媒体条目对应的特征缓存文件路径（``item_key`` 如 ``video/foo``）。"""
    return (cache_dir / item_key).with_suffix(suffix)
