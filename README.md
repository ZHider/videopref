...
媒体(视频文件/图片文件/文件夹/路径列表) ─▶ 🎬 拆帧 Tab ─┬─ 视频 ─▶ frames/video/{名}/0001.jpg ...
                                                          └─ 图片 ─▶ frames/image/{名}.jpg  (单文件, center-crop 到 224)
                                        │ 人工清洗：删除低质帧 / 替换图片 (可选)
                                        ▼
...
frames/
  ...
  image/                    # 图片工作区：一个图片一个文件
    <sanitized名>.jpg          (摄入时 center-crop 到 224×224，与模型输入一致)
```
...
媒体文件/文件夹/路径列表
  ...
  → _make_ingest_handlers       (按 kind 分派)
      ...
      └─ image → ingest_image   → center-crop 到 224×224(与模型输入一致)
...
- **v0.3.2**：**性能优化**——① CUDA 特征提取/推理用 **BF16 半精度**（`torch.autocast`，RTX 5070 Ti 实测约 **1.7x**、特征余弦相似度 0.99995，可 `amp=False` 回退）；② **图片摄入默认缩放到 640 存 JPEG**（`IMAGE_MAX_WIDTH` 与视频帧 `EXTRACT_MAX_WIDTH` 同一宽度标准，大幅节省磁盘/CPU；`FramesNamer` 同步分配 `.jpg` 条目，`0` 可回退原样复制）；③ README 新增「模块依赖与流水线」一节。
